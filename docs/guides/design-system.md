...
- **Monochrome base.** `--primary` is ink: near-black on light, near-white on dark (the CTA is one of the page's few true whites). `--ring`, `--accent`, `--chart-1`, and the sidebar family follow. Hierarchy comes from scale, space, and weight. On the dark field, pure white is a **budgeted material**: headings, buttons/CTAs, and QR paper — everything else builds subtlety from grayscale and alpha steps.
- **The aurora kiss.** `--au-1..5` paint the `.aurora-edge` construction (globals.css, end-of-file block): a 1px mask-composite hairline carrying two counter-drifting conic arcs (9s/13s eased alternates — wavy, never a mechanical spin) plus a 21s linear hue clock, with a blurred `::after` bleed twin (`.aurora-breathe` adds the opacity pulse). This is the marketing color moment and it is rationed: **at most 1 in 3 marketing sections**, attached to the page's highest-intent moments. On the landing that is the hero's URL form; the closing CTA is the only other standing candidate. Reduced motion gets a static stroke. Buttons inside an aurora moment carry weight 500 (the ui Button's default `font-medium` already satisfies this — do not bold them up).
- **Contextual accents.** Scoped color that owns its moment and never joins the system palette: `--dest-1..4` destination identity (dest-1 now pins the old violet literally — it must not follow `--primary` to grey), charts 2-5, `--destructive`. New contextual accents need their own recorded rationale.
...
- **The hero idiom set** (all zero-JS, `globals.css` end block + `components/marketing/hero.tsx`): `hero-stage-*` staged mount entrances (the "considered load": a beat of dark field, then claim → sub → unlit input → the fan dealing out → beam ignite → glow bloom; backwards fill so served markup never hides), `hero-fan`/`hero-mat-1..3`/`hero-paper` (real engine renders on white mats — pose via per-mat custom properties and explicit classes, never nth-child; entrance on the mat, infinite float on the paper, spring hover on the fan), `hero-glow` (steady two-radial aurora under-light), `hero-input` (the aurora-edged URL form; field darker than the panel register so the placeholder reads). Gradient "lit-from-above" strokes on ordinary monochrome controls (D0 note 3) are approved direction whose CSS lands with its first consumer in the section rounds — do not pre-ship dead utilities.

...
- **The attract loop** (P9.10-D13, section 05's plate is the reference implementation — `retarget-stage.tsx`): the second pass's pattern for interactive demos. A demo that wants interaction runs ITSELF until touched: a timed cycle drives the same state the visitor's controls drive (05 retargets itself to the next destination every 4s — branch, chips, and the destination window all follow), and the first real interaction cancels the loop permanently, so the visitor keeps the keys. Two rules are load-bearing. (1) **The machine's actions never count toward a visitor-proof device**: while it lived, 05's retarget counter stayed at 0 however long the loop ran, because that counter was the visitor's own proof of D14 and a machine inflating it would be fabrication. (The counter itself retired at D13.1 on a board note — its facts moved into the flanking claims — but the rule binds any future visitor-proof device under an attract loop.) (2) **Three gates**: in-view only (IntersectionObserver), never under reduced motion (matchMedia, watched in both directions — the 03 rule), torn down for good on first interaction. `data-attract="on"/"off"` on the plate root is the honest is-the-machine-driving signal, SSR'd as `off`, and it is what e2e asserts instead of waiting on timers. What the loop shows since D13.1 is **the connect pulse**: dotted branch bases in 04's spoke idiom (routes that could be), and the active branch as a keyed draw-in overlay in the destination's own hue — `pathLength=100`, `stroke-dasharray: 100 100`, offset animated 100→0 in a stylesheet-gated keyframe — so the pulse head travels code→chip, the lit line holds behind it, the old connection drops the frame the new one departs, and the same 500ms set serves every branch length. Rest state outside the gate is the drawn line, so reduced motion never sees a bare board. 05's plate also wears 04's sandbox material (border-border/60 + bg-card/25 + the 24px border-token dot lattice, D13.1) — that material is now a two-consumer idiom for "demonstration canvas", not a 04 one-off.
...
| `../marketing/studio-object.tsx` (+ `lib/webgl/*`) | `StudioObject` | No (`"use client"`; SSRs its real-engine fallback mat) | The landing's true-3D QR slab (P9.10-D11): raw WebGL1, zero deps — embossed engine raster on an extruded beveled slab, aurora-tinted specular under Reinhard headroom (the answer to CSS's white-on-white sheen failure), cursor tilt via `lib/spring.ts` + `tilt-math.ts`, idle drift, sweep-swap on config change. Landing section 03 only; see "Studio vs. marketing staging" below for how it amends the round-3 split |
...
- **D11 amendment (board directive): landing section 03 now stages an interactive 3D object too — but in WebGL, not by importing `TiltStage`.** The round-3 split's REASON stands (a screenshot-like marketing visual is looked *at*; the split was never "marketing must be flat"), and the board's second-pass brief for 03 was explicitly a true-3D object with a true light sheen: `TiltStage`'s CSS sheen is structurally invisible on white paper (you can only add white to white — its own round-4/5 history), which is the exact failure `StudioObject`'s shader solves with tone-mapping headroom. So the rule now reads: `TiltStage` stays studio-only; `StudioObject` is landing-03-only; `ArtifactStage` remains the default marketing rig. Three rigs, one surface each — never mix them on one surface.

...
- **The attract loop** (P9.10-D13, section 05's plate is the reference implementation — `retarget-stage.tsx`): the second pass's pattern for interactive demos. A demo that wants interaction runs ITSELF until touched: a timed cycle drives the same state the visitor's controls drive (05 retargets itself to the next destination every 4s — branch, chips, and the destination window all follow), and the first real interaction cancels the loop permanently, so the visitor keeps the keys. Two rules are load-bearing. (1) **The machine's actions never count toward a visitor-proof device**: 05's retarget counter stays at 0 however long the loop runs, because that counter is the visitor's own proof of D14 and a machine inflating it would be fabrication. (2) **Three gates**: in-view only (IntersectionObserver), never under reduced motion (matchMedia, watched in both directions — the 03 rule), torn down for good on first interaction. `data-attract="on"/"off"` on the plate root is the honest is-the-machine-driving signal, SSR'd as `off`, and it is what e2e asserts instead of waiting on timers. The destination window that gives the loop something to show (a browser-idiom card, full current URL in the address strip, an abstract per-destination wireframe body — grey blocks only, never fake content) swaps by keyed remount with a stylesheet-gated enter, so only the active wireframe is ever in the served DOM.

...
- `next-themes` (`apps/web/components/theme-provider.tsx` wraps `ThemeProvider` from `next-themes`) is mounted in `app/layout.tsx` with `attribute="class" defaultTheme="system" enableSystem enableColorScheme={false}`, so it toggles the `.dark` class on `<html>` and respects OS preference by default. `<html>` also has `suppressHydrationWarning` since the class is set client-side post-hydration. The UA `color-scheme` property is CSS-driven (`globals.css`, next to the base layer) rather than next-themes' inline style — an inline style would defeat the marketing forced-dark override below.
...
### Marketing is dark-only (board directive, 2026-08-06 / P9.9-C0.5)

- **Every marketing surface renders the dark register regardless of the visitor's OS or site theme**; there is no theme toggle on marketing chrome. The board's rationale, verbatim intent: designing every marketing section twice adds complication for no gain, dark carries the glow language better, and white QR codes pop harder on a dark page (QR's black-ink-on-white convention makes the inverted mat the distinctive one).
- Mechanism: a `dark`-classed wrapper carrying `data-force-dark` in `app/(marketing)/layout.tsx`, `app/not-found.tsx`, and `/login`'s shell, plus `html:has([data-force-dark])` rules in `globals.css` for the two things a wrapper cannot reach (UA `color-scheme`; the `<body>` background behind overscroll). Body-level PORTALS also escape the wrapper — portaled content used on marketing carries its own `dark` class (SiteNav's Features dropdown, Playground's Download popover). Any new marketing portal must do the same.
- **The app keeps light/dark as a real preference**: the ThemeToggle (now `components/theme-toggle.tsx`) lives in AppNav. The public /studio is an app surface and follows the visitor's theme (`marketing.spec.ts` guards both directions).
- **"Reversed" sections are a design tool, not a theme**: on the dark-primary page, an occasional hard-coded LIGHT section (a "paper plate", the inverse of `surface="ink"`) is available vocabulary for alternation where it earns its place — chosen per section in a C-round exploration, never user-toggleable. Note the inversion consequence: on a dark page the ink plate reads as a subtle deepening, so the paper plate is now the loud surface move.
- The pre-rendered light/dark QR SVG pairs (`qr-tile.tsx`, `filmstrip.tsx` `dark:hidden`/`dark:block`) still work under forced dark; the light variant is dead payload on marketing and can be stripped opportunistically as C rounds touch each section.
